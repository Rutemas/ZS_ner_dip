...
class InputExample(object):
    ...
    def get_words_segment(self, spm, lang):
        if lang in CHARACTER_SPLIT_LANGS:
            ...
            for i in range(len(self.tokenized_words)):
                ...
                #or self.tokenized_words[i] in [',','.','px']:
                    if len(self.words_idxs) > 0:
                        self.words_idxs[-1].append(i)
                    self.words_idxs.append([i])
                #else:
                    #print("get_words_segment if i",i)
                    #print("get_words_segment self.words_idxs", self.words_idxs)
    ...
    def is_entity(self, i):
        #print(f"is_entity self.labels[{i}]",self.labels[i])
    ...
    def insert_with_slot(self, spm, lang):
        if lang in CHARACTER_SPLIT_LANGS:
            ...
            #print("self.raw_sentence",self.raw_sentence)
            self.tokenized_sentence = spm.encode(self.raw_sentence)
            #print("self.tokenized_sentence",self.tokenized_sentence)
            self.tokenized_words = self.tokenized_sentence.split()
            #print("self.tokenized_words",self.tokenized_words)
            self.tokenized_masked_words = self.tokenized_sentence.split()
            self.get_words_segment(spm, lang)
            #print("self.words_idxs",self.words_idxs)
            self.entity_number = 0
            #print("len(self.words)",len(self.words))
            ...
            for i in range(len(self.words)):
                if self.is_beginning_of_entity(i) or (self.is_entity(i) and i == 0):
                    ...
                    #print("self.entities[-1] ",self.entities[-1])
                    #print("self.entities ",self.entities)
                    #print("self.labels[i]",self.labels[i])
                    #print("i",i)
                    #print("self.words_idxs[i][0] ",self.words_idxs[i][0])
                    #print("self.words_idxs[i][1] ",self.words_idxs[i][1])
                    #print("self.words_idxs[i]", self.words_idxs[i])
                    #print("self.words_idxs", self.words_idxs)
...
if __name__ == "__main__":
    ...
    for index, example in enumerate(examples):
        ...
        #print("slot_sentence",slot_sentence)
        #print("entity",entity)
