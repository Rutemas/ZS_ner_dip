...
fh = logging.FileHandler(
    "m2m/shells/m2m/UniTrans/ner/generate_pseudo.log")
...
def calculate_recall(groundtruth_labels, pred_labels):
    groundtruth_labels = get_list(groundtruth_labels)
    pred_labels = get_list(pred_labels)
    return recall_score(groundtruth_labels, pred_labels)
    
def calculate_f1(groundtruth_labels, pred_labels):
    groundtruth_labels = get_list(groundtruth_labels)
    pred_labels = get_list(pred_labels)
    return f1_score(groundtruth_labels, pred_labels)
...
if __name__ == "__main__":
    ...
    for idx, (translated_example, groundtruth_example, x_example) in enumerate(
        ...
        count += 1
    logger.info(f"len(groundtruth_labels), len(ner_labels) {len(groundtruth_labels), len(ner_labels)}")
    logger.info(f"Prediction Precision {calculate_precision(groundtruth_labels, ner_labels)}")
    logger.info(f"Prediction Recall {calculate_recall(groundtruth_labels, ner_labels)}")
    logger.info(f"Prediction F1 {calculate_f1(groundtruth_labels, ner_labels)}")
